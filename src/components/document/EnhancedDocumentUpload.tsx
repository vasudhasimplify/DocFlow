  ...
  const uploadSingleFile = async (uploadFile: UploadFile): Promise<string | null> => {
    try {
      ...
      if (enablePolicyDetection) {
        ...
        console.log('🔍 Checking policy keywords for file:', fileName);
        console.log('🔍 Keywords to match:', POLICY_KEYWORDS);
        
        const matchedKeywordInName = POLICY_KEYWORDS.find(k => fileName.includes(k));
        const isPolicyByName = !!matchedKeywordInName;
        console.log('🔍 Keyword matched in filename:', matchedKeywordInName || 'none');
        ...
        console.log('🔍 Keyword matched in content:', matchedKeywordInContent || 'none');
        ...
        if (isPolicyByName || isPolicyByContent) {
          ...
          updateFileStatus(uploadFile.id, { isPolicyDocument: true });
          
          // Dispatch event to SimplifyDrive to show policy detection dialog
          // This ensures the dialog persists even after upload modal closes
          setTimeout(() => {
          ...
          console.log('⚠️ No policy keywords matched for:', fileName);
        }
      } else {
        console.log('⚠️ Policy detection is disabled');
      }
