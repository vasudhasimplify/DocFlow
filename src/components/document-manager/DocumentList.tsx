        ...
        {documents.map(document => (
          <ContextMenu key={document.id}>
            <ContextMenuTrigger asChild>
              <Card
                className={`group hover:shadow-md transition-all duration-200 cursor-pointer border hover:border-primary/50 relative ${selectedDocuments.has(document.id) ? 'border-primary border-2 bg-primary/5' : 'bg-white'
                  }`}
                onClick={(e) => {
                  e.stopPropagation();
